...
pthread_t tid[MAX_CLIENTS];  //in pthread.h header       //array to store the thread ids of clients
...
int sendErrorMessage(int socket, int status_code)
{
	char str[1024];
	char currentTime[50];
	time_t now = time(0); //Gets the current time in seconds since the epoch (January 1, 1970).

	struct tm data = *gmtime(&now);
	strftime(currentTime,sizeof(currentTime),"%a, %d %b %Y %H:%M:%S %Z", &data); //Formats the tm structure into a string stored in currentTime, representing the date and time in a standard HTTP format (e.g., "Mon, 27 Aug 2023 14:55:29 GMT").
...
int connectRemoteServer(char* host_addr, int port_num)
{
	...
	server_addr.sin_port = htons(port_num); //from integer to required binary form 
...
int handle_request(int clientSocket, ParsedRequest *request, char *tempReq)
{
	...
	char *temp_buffer = (char*)malloc(sizeof(char)*MAX_BYTES); //temp buffer to store the response in the cache
...
void* thread_fn(void* socketNew)
{
	...
	// reserves n(MAX_BYTES) blocks of memory with the given amount of bytes (sizeof(char)) , initializes them with 0
	...
	if( temp != NULL){
		...
		int size=temp->len/sizeof(char); //temp->len is size of the temp and size is actually the number of elements
		int pos=0;
	...
	close(socket);
	free(buffer);
	sem_post(&seamaphore);	// or sem_signal
...
int main(int argc, char * argv[]) {  //char *argv[] is essentially an array where each element is a pointer to a char. Each pointer in this array points to the beginning of a string.
    ...
    sem_init(&seamaphore,0,MAX_CLIENTS); // Initializing seamaphore and lock , 0 is the min-client
	...
	proxy_socketId = socket(AF_INET, SOCK_STREAM, 0);// it returns a file descriptor
    ...
    // proxy_socketId is declared globally and once it is created we have to reuse it
	int reuse =1; 
	if (setsockopt(proxy_socketId, SOL_SOCKET, SO_REUSEADDR, (const char*)&reuse, sizeof(reuse)) < 0) //The option name to set. SO_REUSEADDR allows the socket to bind to an address that is in a TIME_WAIT state, which is useful for quickly restarting a server on the same port.
        perror("setsockopt(SO_REUSEADDR) failed\n");

   //generic data type so we can type cast to any data type
	bzero((char*)&server_addr, sizeof(server_addr));  
	server_addr.sin_family = AF_INET;
	...
	while(1)
	{
		...
		inet_ntop( AF_INET, &ip_addr, str, INET_ADDRSTRLEN ); //It converts a network address in binary form (such as that held in a struct in_addr for IPv4 addresses) into a string format (e.g., "192.168.1.1") that is easy to read and display. and then stores it in str variable
